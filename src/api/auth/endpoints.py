...
async def register(service: AuthServiceDependency, data: RegisterRequest = Body()) -> JSONResponse:
    user = await service.register(data.name, data.username, data.password)
    return JSONResponse(content=SuccessResponse(message=f"Пользователь {user.name} успешно создан").model_dump(), status_code=status.HTTP_200_OK)


@router.get("/me", response_model=UserResponseSchema)
async def get_me(user: CurrentUserDependency) -> JSONResponse:
    if not user:
        return JSONResponse(content=ErrorResponse("Пользователя не существует").model_dump(), status_code = status.HTTP_401_UNAUTHORIZED)
